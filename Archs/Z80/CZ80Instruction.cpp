...
bool CZ80Instruction::Validate(const ValidateState& state)
{
	...
	if (Opcode.flags & Z80_HAS_IMMEDIATE)
	{
		...
		if (Vars.Immediate < min || Vars.Immediate > max)
		{
			if (Opcode.flags & Z80_JUMP_RELATIVE)
			{
				Logger::queueError(Logger::Error, "Jump target %04X out of range", Vars.Immediate);
			}
			else if (Opcode.flags & Z80_INTERRUPT_MODE)
			{
				Logger::queueError(Logger::Error, "Interrupt mode %i out of range", Vars.Immediate);
			}
			else
			{
				Logger::queueError(Logger::Error, "Immediate %i out of range", Vars.Immediate);
			}
			return false;
		}
		if ((Opcode.flags & Z80_HAS_2_IMMEDIATES) && (Vars.Immediate2 < min2 || Vars.Immediate2 > max2))
		{
			Logger::queueError(Logger::Error, "Immediate %i out of range", Vars.Immediate2);
			return false;
		}
		...
		if (Opcode.flags & Z80_RST)
		{
			...
			if (Z80.GetVersion() == Z80ArchType::MARCH_Gameboy && Vars.Immediate >= 0x00 && Vars.Immediate <= 0x7)
			{
				// Nintendo syntax
				// 1 -> 8, 2 -> 16, 3 -> 24, etc
				Vars.Immediate <<= 3;
			}
			else if (Z80.GetVersion() == Z80ArchType::MARCH_Ereader && Vars.Immediate != 0x00 && Vars.Immediate != 0x08)
			{
				invalidRST = true;
			}
			...
			if (invalidRST)
			{
				Logger::queueError(Logger::Error, "Invalid RST target %i", Vars.Immediate);
				return false;
			}
		}
