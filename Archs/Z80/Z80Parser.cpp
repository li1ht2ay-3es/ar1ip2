...
const Z80RegisterDescriptor Z80Regs8[] = {
	{ "b", Z80_REG8_B }, { "c", Z80_REG8_C },
	{ "d", Z80_REG8_D }, { "e", Z80_REG8_E },
	{ "h", Z80_REG8_H }, { "l", Z80_REG8_L },
	{ "a", Z80_REG8_A },
};

const Z80RegisterDescriptor Z80Regs16SP[] = {
	{ "bc", Z80_REG16_BC }, { "de", Z80_REG16_DE },
	{ "hl", Z80_REG16_HL }, { "sp", Z80_REG16_SP },
};

const Z80RegisterDescriptor Z80Regs16AF[] = { // kinda hacky
	{ "bc", Z80_REG16_BC }, { "de", Z80_REG16_DE },
	{ "hl", Z80_REG16_HL }, { "af", Z80_REG16_AF },
};

const Z80RegisterDescriptor Z80Regs16IXIY[] = { // kinda hacky
	{ "bc", Z80_REG16_BC }, { "de", Z80_REG16_DE },
	{ "ix", Z80_REG16_IX }, { "iy", Z80_REG16_IY },
	{ "sp", Z80_REG16_SP },
};
...
const Z80RegisterDescriptor Z80Conds[] = {
	{ "nz", Z80_COND_NZ }, { "z",  Z80_COND_Z },
	{ "nc", Z80_COND_NC }, { "c",  Z80_COND_C },
	{ "po", Z80_COND_PO }, { "pe", Z80_COND_PE },
	{ "p",  Z80_COND_P },  { "m",  Z80_COND_M },
};

const Z80RegisterDescriptor Z80RegsIR[] = {
	{ "i", Z80_REG_I }, { "r", Z80_REG_R },
};
...
bool Z80Parser::parseRegisterTable(Parser& parser, Z80RegisterValue& dest, const Z80RegisterDescriptor* table, size_t count, int allowed)
{
	...
	{
		if (allowed & (1 << table[i].num) && identifier.string() == table[i].name)
		{
			dest.name = identifier.string();
...
bool Z80Parser::parseRegisterAFShadow(Parser& parser)
{
	...
	CHECK(token.type == TokenType::Identifier);
	CHECK(token.identifierValue().string() == "af");
	CHECK(parser.nextToken().getOriginalText() == "'");
...
bool Z80Parser::parseCondition(Parser& parser, Z80RegisterValue& dest)
{
	...
	if (Z80.GetVersion() == Z80ArchType::MARCH_Gameboy)
	{
		allowed = Z80_COND_BIT_GB;
	}
	else if (Z80.GetVersion() == Z80ArchType::MARCH_Ereader)
	{
...
std::unique_ptr<CZ80Instruction> Z80Parser::parseOpcode(Parser& parser)
{
	...
	for (int z = 0; Z80Opcodes[z].name != nullptr; z++)
	{
		...
		if (identifier.string() == Opcode.name)
		{
	...
	if (paramFail)
		parser.printError(token, "%s parameter failure in %S", Z80.GetName(), identifier);
	else
		parser.printError(token, "Invalid %s opcode: %S", Z80.GetName(), identifier);
